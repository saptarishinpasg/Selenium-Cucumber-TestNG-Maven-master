...
import Common.FetchDataFromCSV;
import Common.FetchDataFromExcel;
...
public class webconnector<V> {
	...
	public webconnector()
	{
    	try {
    		...
    		envprop.load(new FileInputStream("./src/test/config/environment.properties"));
    	} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	System.out.println("Inside click Element Exception");
    }
    }
    ...
    public By getElementWithXpath(WebElement element, String txt) throws Exception {
		String locatorTypeAndValue = element.getAttribute(txt);		
		return By.xpath(locatorTypeAndValue);		
		
    }
    
    
    public void fetchTestData(String TestCaseName, String FileName) throws InvalidFormatException, IOException, CsvException {
    	String fullPath = prop.getProperty("testDataBasePath")+"\\"+prop.getProperty("testDataFormat")+"\\"+FileName;
    	
    	if(prop.getProperty("testDataFormat").equals("xlsx"))
    		testData = FetchDataFromExcel.testDataFromExcel(fullPath, TestCaseName);
    	else if(prop.getProperty("testDataFormat").equals("csv"))
    		testData = FetchDataFromCSV.testDataFromCsv(fullPath, TestCaseName);
    	else
    		System.out.println("File extension not supported.");
    }
    
    public void goToSite(String site, String env) throws InvalidFormatException, IOException{
    	String url=envprop.getProperty(env+"."+site+".url");

    	if(url != null) {
    		System.out.println("URL found: " + url);
            driver.get(url);
            waitForCondition("PageLoad","",60);
    	}
    	else
